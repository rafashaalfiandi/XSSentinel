...
def source_priority_label(source: str) -> tuple[str, str]:
    ...
    labels = {
        "direct": ("[GET]", f"{CYAN}DIRECT{RESET}"),
        "direct-all-params": ("[ALL]", f"{YELLOW}ALL-PARAMS{RESET}"),
        "high-value-form": ("[HIGH]", f"{GREEN}HIGH-VALUE{RESET}"),
        "form": ("[FORM]", f"{YELLOW}FORM{RESET}"),
        "graphql": ("[API]", f"{MAGENTA}GRAPHQL{RESET}"),
        "fetch": ("[FETCH]", f"{MAGENTA}FETCH{RESET}"),
        "axios": ("[AXIOS]", f"{MAGENTA}AXIOS{RESET}"),
        "xhr": ("[XHR]", f"{MAGENTA}XHR{RESET}"),
        ...
        "spa-route": ("[SPA]", f"{MAGENTA}SPA-ROUTE{RESET}"),
        "js-route": ("[JS]", f"{MAGENTA}JS-ROUTE{RESET}"),
...
def target_scan_mode(target: ScanTarget) -> str:
    source = getattr(target, "source", "")
    if source == "direct-all-params":
        return "all-params"
    return "single-param"

def scan_mode_description(mode: str) -> str:
    if mode == "all-params":
        return "all query parameters receive the same payload in each request"
    return "one parameter is fuzzed per request"

def print_scan_mode_summary(targets: list[ScanTarget], args: argparse.Namespace) -> None:
    if not targets:
        return
    modes = {target_scan_mode(target) for target in targets}
    mode = "mixed" if len(modes) > 1 else next(iter(modes))
    if mode == "mixed":
        detail = "mixed direct/discovered target strategy"
    else:
        detail = scan_mode_description(mode)
    total_fuzz = sum(len(target.fuzz_locations) for target in targets)
    stop_policy = "first-confirmed" if getattr(args, "stop_on_confirmed", False) else "per-target-confirmed"
    print(f"{info_tag()} {info_key('scan-mode')}{mode} {DIM}({detail}){RESET}")
    print(f"{info_tag()} {info_key('injection-points')}{total_fuzz} across {len(targets)} endpoint(s)")
    print(f"{info_tag()} {info_key('stop-policy')}{stop_policy}")
...
def print_endpoint_discovery_summary(
    ...
    direct_get = [t for t in get_targets if getattr(t, 'source', '') in ("direct", "direct-all-params")]
    direct_split = [t for t in direct_get if getattr(t, 'source', '') == "direct"]
    direct_all = [t for t in direct_get if getattr(t, 'source', '') == "direct-all-params"]
    ...
    js_post = [t for t in post_targets if getattr(t, 'source', '') in ("js-post", "js-post-json", "fetch", "axios", "xhr", "graphql")]
    links = [t for t in get_targets if getattr(t, 'source', '') == "link"]
    standalone = [t for t in get_targets if getattr(t, 'source', '') in ("html-standalone", "js-single", "grouped", "spa-route", "js-route", "fetch", "axios", "xhr")]
    ...
    if direct_get and len(direct_get) == len(targets):
        discovery_mode = "direct-target"
    else:
        discovery_mode = "auto-discovery"

    print()
    print(f"{info_tag()} {BOLD}endpoints={len(targets)} mode={discovery_mode}{payload_info}{RESET}")
    print(f"{info_tag()} {info_key('base')}{base}")

    if direct_get and len(direct_get) == len(targets):
        print(f"{info_tag()} {CYAN}priority=direct GET parameter target(s){RESET}")
    elif post_targets:
    ...
    for t in targets:
        ...
        all_display_targets.append({
            ...
            'order': (0 if t.method == "POST" else 1,
                     {"high-value-form": 0, "form": 1, "graphql": 2, "fetch": 3, "axios": 4, "xhr": 5, "js-post-json": 6, "js-post": 7, "direct-all-params": 8, "direct": 9, "html-standalone": 10, "link": 11, "js-single": 12, "grouped": 13, "spa-route": 14, "js-route": 15, "fallback": 16, "unknown": 17}.get(source, 18))
        })
    ...
    summary_parts = []
    if direct_all:
        summary_parts.append(f"{YELLOW}ALL={len(direct_all)}{RESET}")
    if direct_split:
        summary_parts.append(f"{CYAN}DIRECT={len(direct_split)}{RESET}")
    if high_value:
    ...
    if direct_get and len(direct_get) == len(targets):
        insights.append(f"{CYAN}Direct target split into {len(get_targets)} GET parameter target(s){RESET}")
    elif not post_targets:
...
def print_scan_info(
    ...
    dom_enabled = "on" if intel.script_analysis.suspicious_scripts or intel.script_analysis.inline_scripts or intel.script_analysis.external_scripts else "off"
    ...
    fields = [
        ...
        info_field("scan-mode", target_scan_mode(target)),
...
def print_result(number: int, finding: Finding, agent: str | None = None) -> None:
    ...
    detail = ""
    if status in {"API_REFLECTED", "API_RISK"}:
        detail = "API response reflects payload; browser confirmation required"
        if finding.reflections:
            strongest = max(finding.reflections, key=lambda item: item.severity)
            detail = f"{detail} DOM={strongest.context}:{strongest.detail}"
    elif finding.reflections:
        strongest = max(finding.reflections, key=lambda item: item.severity)
    ...
    if finding.browser_evidence and status != "CONFIRMED" and not error:
        evidence_label = "browser" if finding.browser_confirmed else "evidence"
        detail = f"{detail} {evidence_label}={finding.browser_evidence}"
    if finding.confidence:
        detail = f"{detail} confidence={finding.confidence}"
    if status == "CONFIRMED":
        target = f"{payload_label(finding.payload)} evidence=\"{one_line(finding.browser_evidence or 'browser-confirmed', 80)}\""
    else:
    ...
    if status in {"API_REFLECTED", "API_RISK"}:
        print(f"  {CYAN}url:{RESET} {finding.url}")
        print(f"  {CYAN}payload:{RESET} {payload_label(finding.payload)}")
...
def print_summary(findings: list[Finding]) -> None:
    ...
    api_count = counts.get("API_REFLECTED", 0) + counts.get("API_RISK", 0)
    if confirmed:
        finding = confirmed[0]
        print()
        ...
        print(
            f"  {result_key('stats')} confirmed={counts.get('CONFIRMED', 0)} api={api_count} "
        ...
        print(f"  {result_key('payload')} {payload_label(finding.payload)}")
        ...
        print(f"  {result_key('confidence')} {finding.confidence}/100")
    ...
    api_findings = [f for f in findings if rendered_status(f) in {"API_REFLECTED", "API_RISK"}]
    if api_findings:
        finding = api_findings[0]
        print(f"  {result_key('api')} {rendered_status(finding)}")
        print(f"  {result_key('evidence')} {one_line(finding.browser_evidence or 'api evidence', 180)}")
        print(f"  {result_key('confidence')} {finding.confidence}/100")
        print(f"  {result_key('payload')} {payload_label(finding.payload)}")
        print(f"  {result_key('url')} {finding.url}")
