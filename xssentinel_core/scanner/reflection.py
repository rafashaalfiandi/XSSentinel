...
def looks_like_active_payload_document(body: str, payload: str) -> bool:
    stripped = html.unescape(body).strip()
    variants = [html.unescape(item).strip() for item in payload_match_variants(payload)]
    variants = [item for item in variants if item]
    if not stripped or not variants:
        return False
    for decoded_payload in variants:
        if stripped == decoded_payload:
            return True
        if len(stripped) <= max(500, len(decoded_payload) + 200) and decoded_payload in stripped:
            return True
        if decoded_payload in stripped and re.search(r"^\s*(?:<!doctype\s+html|<html\b|<script\b|<svg\b|<iframe\b)", stripped, re.I):
            return True
    return False
...
def is_download_or_api_delivery(headers: dict[str, str], url: str) -> bool:
    ...
    content_type = response_header_value(headers, "Content-Type").lower()
    parsed = urllib.parse.urlsplit(url)
    url_hint = " ".join([parsed.path, parsed.query]).lower()
    if "attachment" in disposition or "filename=" in disposition:
        return True
    if any(token in content_type for token in ("text/html", "application/xhtml", "image/svg", "text/javascript", "application/javascript")):
        return True
...
def delivered_payload_evidence(payload: str, body: str, headers: dict[str, str], url: str) -> str:
    ...
    if looks_like_active_payload_document(body, payload):
        return "response body is an active XSS payload document"
