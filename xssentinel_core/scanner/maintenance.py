...
def reinstall_from_source(source_dir: Path) -> None:
    required_files = ("main.py", "useragents.txt")
    missing = [source_dir / filename for filename in required_files if not (source_dir / filename).is_file()]
    core_source = source_dir / "xssentinel_core"
    if not core_source.is_dir():
        missing.append(core_source)
    if missing:
        formatted = ", ".join(str(path) for path in missing)
        raise SystemExit(f"{RED}{BOLD}[ERROR]{RESET} Missing required source path(s): {formatted}")
