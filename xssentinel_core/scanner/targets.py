...
from .crawler import EndpointCandidate, RecursiveCrawler, extract_parameter_names as crawler_extract_parameter_names
...
def build_single_query_fuzz_url_by_index(template_url: str, fuzz_index: int) -> str:
    parsed = urllib.parse.urlsplit(template_url)
    parts = []
    for index, item in enumerate(parsed.query.split("&") if parsed.query else []):
        key, sep, value = item.partition("=")
        decoded_key = urllib.parse.unquote_plus(key)
        decoded_value = urllib.parse.unquote_plus(value)
        if sep and decoded_value == "FUZZ":
            replacement = "FUZZ" if index == fuzz_index else benign_value_for_field(decoded_key)
            parts.append(f"{key}={urllib.parse.quote_plus(replacement)}")
        else:
            parts.append(item)
    return urllib.parse.urlunsplit((parsed.scheme, parsed.netloc, parsed.path, "&".join(parts), parsed.fragment))
...
def extract_candidate_params_from_text(text: str) -> list[str]:
    candidates: list[str] = crawler_extract_parameter_names(text)
    patterns = [
...
def split_get_targets_by_parameter(targets: list[ScanTarget]) -> list[ScanTarget]:
    ...
    for target in targets:
        ...
        fuzz_indexes: list[int] = []
        if target.method == "GET":
            for index, item in enumerate(parsed.query.split("&") if parsed.query else []):
                _, sep, value = item.partition("=")
                if sep and urllib.parse.unquote_plus(value) == "FUZZ":
                    fuzz_indexes.append(index)
        if len(fuzz_indexes) <= 1:
            result.append(target)
            continue
        for index in fuzz_indexes:
            split_target = ScanTarget(
                "GET",
                build_single_query_fuzz_url_by_index(target.url, index),
                None,
                target.content_type,
                [],
                target.source,
            ...
            split_target.discovery_confidence = getattr(target, "discovery_confidence", 0)
            split_target.discovery_evidence = list(getattr(target, "discovery_evidence", []))
            split_target.crawl_depth = getattr(target, "crawl_depth", 0)
...
def target_priority_key(t: ScanTarget) -> tuple[int, int, str]:
    ...
    return (0 if t.method == "POST" else 1, source_order, f"{-getattr(t, 'discovery_confidence', 0):03d}:{t.url}")

def attach_discovery_metadata(target: ScanTarget, candidate: EndpointCandidate) -> ScanTarget:
    target.source = candidate.source
    target.discovery_confidence = candidate.confidence
    target.discovery_evidence = candidate.evidence
    target.crawl_depth = candidate.depth
    return target

def candidate_to_scan_targets(candidate: EndpointCandidate) -> list[ScanTarget]:
    fields = fuzz_candidate_fields(candidate.parameters[:12])
    if not fields:
        return []
    targets: list[ScanTarget] = []
    if candidate.method == "POST":
        ...
        for field in fields:
            data = json_data_template(candidate.parameters[:12], field) if content_type == "application/json" else form_data_template(candidate.parameters[:12], field)
            target = ScanTarget("POST", normalize_query_values_to_fuzz(candidate.url), data, content_type, [], candidate.source)
            target.fuzz_locations = target_fuzz_locations(target)
            if target.fuzz_locations:
                targets.append(attach_discovery_metadata(target, candidate))
        return targets

    if query_params(candidate.url):
        target = query_link_to_target(candidate.url)
        if target:
            target.source = candidate.source
            targets.append(attach_discovery_metadata(target, candidate))
    else:
        for field in fields:
            target = field_names_to_get_target(candidate.url, [field])
            if target:
                target.source = candidate.source
                targets.append(attach_discovery_metadata(target, candidate))
    return targets

def crawler_candidates_to_targets(candidates: list[EndpointCandidate]) -> list[ScanTarget]:
    targets: list[ScanTarget] = []
    for candidate in candidates:
        targets.extend(candidate_to_scan_targets(candidate))
    return targets
...
def discover_form_targets(
    ...
    seed_result = HttpResult(status, body, {}, error)
    crawler = RecursiveCrawler(
        user_agent,
        timeout,
        verify_https,
        extra_headers or {},
        CrawlerRuntimeConfig.from_env(),
    )
    crawl_result = crawler.crawl(resolved_url, seed_result)
    targets = crawler_candidates_to_targets(crawl_result.endpoints)

    if not targets:
        discovery_text = "\n".join(page.body[:30000] for page in crawl_result.pages if page.body) or body[:30000]
        fallback = fallback_common_target(resolved_url, discovery_text)
        if fallback:
            fallback.source = "fallback"
            fallback.discovery_confidence = 10
            fallback.discovery_evidence = ["fallback parameter names"]
...
def make_scan_targets(args: argparse.Namespace, user_agent: str) -> list[ScanTarget]:
    ...
    if (
        getattr(args, "all_params", False)
        and query_params(args.url)
        and not body_has_fuzz(args.data)
        and args.method.upper() in {"AUTO", "GET"}
    ):
        resolved_url = resolve_scan_url_candidate(args.url, user_agent, args.timeout, args.verify_https, args.headers)
        args.url = normalize_query_values_to_fuzz(resolved_url)
        target = make_scan_target(args)
        target.source = "direct-all-params"
        return [target]
