...
def is_valid_download_status(status: str) -> bool:
    return status in {"CONFIRMED", "DOWNLOAD_CONFIRMED"}
...
def scan_target_worker(
    ...
    for batch_name, batch_payloads in batches:
        ...
        for payload_index, payload in enumerate(iter_payloads(batch_payloads, None if args.smart else args.limit), start=1):
            ...
            context_snippets = html_context_snippets(body, payload) if body and (reflections or download_evidence) else []
            browser_confirmed = False
            browser_evidence = download_evidence
            if not download_evidence and should_verify_with_browser(payload, reflections, args) and browser_checks_done < browser_validation_budget(payload, reflections, args):
            ...
            if args.stop_on_confirmed and is_valid_download_status(status):
                stop_event.set()
                target_done = True
                break
